...
export const api = {
	...
	showDialog: async (
		...
		isError: boolean = false, isCrash: boolean = false
	) => {
		if (!buttons) {
			buttons = ["Ok"];
		}
	...
	updateSpriteMenu: async(flags: boolean[]) => {
		return await send("updateSpriteMenu", flags);
	},

	mountMapMenu: async(flags: boolean[]) => {
